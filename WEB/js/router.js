    ...
    if (!IS_TEST_MODE && !socket) {
       ...
       // Глобальный обработчик для timer_end
        ...
        socket.on('revelation_complete', (data) => {
            window._revelationCompleted = false; // Сбрасываем флаг для следующего раунда
            currentGamePhase = 'voting';
            ...
            // Не блокируем навигацию полностью, только переход на профиль
            if (typeof blockNavigation === 'function') {
                blockNavigation(true, document.querySelector('.container'));
            }
...
function blockNavigation(block, container) {
    ...
    const voteIcon = document.querySelector('.icon-left');
    const cardsIcon = document.querySelector('.icon-center');
    const cards = document.querySelectorAll('.profile-card');

    if (block) {
        // Блокируем иконку ПРОФИЛЯ только на этапе голосования
        if (profileIcon) {
            if (currentGamePhase === 'voting') {
                ...
                newIcon.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    showToast('Сейчас этап голосования! Переход на профиль заблокирован.', 'warning');
                });
        ...
        // Блокируем иконку ГОЛОСОВАНИЯ на этапе вскрытия карт И после раунда
        if (voteIcon) {
            if (currentGamePhase === 'revelation' || currentGamePhase === 'waiting') {
                voteIcon.style.pointerEvents = 'none';
        ...
        // Иконка "Карточки всех игроков" ВСЕГДА активна
        if (cardsIcon) {
            cardsIcon.style.pointerEvents = 'auto';
            cardsIcon.style.opacity = '1';
        }
        ...
        // Блокировка карт (только на этапе вскрытия, когда НЕ ваш ход)
    } else {
        ...
        // Разблокируем иконку голосования
         if (voteIcon) {
            ...
            const newVoteIcon = voteIcon.cloneNode(true);
            voteIcon.parentNode.replaceChild(newVoteIcon, voteIcon);
            newVoteIcon.addEventListener('click', () => {
                if (currentGamePhase === 'revelation') {
                    return;
                }
                if (timerInterval) clearInterval(timerInterval);
...
function addPageHandlers(container) {
    ...
    if (isProfilePage) {
        ...
        // Блокируем навигацию в зависимости от фазы игры
        if (currentGamePhase === 'revelation') {
            // На этапе вскрытия карт - блокируем ТОЛЬКО иконку голосования
            blockNavigation(true, container);
            // Принудительно разблокируем иконку профиля
            const profileIcon = document.querySelector('.icon-right');
            if (profileIcon) {
                profileIcon.style.pointerEvents = 'auto';
                profileIcon.style.opacity = '1';
            ...
            // На этапе голосования - блокируем иконку профиля
            blockNavigation(true, container);
            // Принудительно разблокируем иконку голосования
            const voteIcon = document.querySelector('.icon-left');
            if (voteIcon) {
                voteIcon.style.pointerEvents = 'auto';
                voteIcon.style.opacity = '1';
    ...
    if (playersContainerCheck) {
        ...
        // СИНХРОНИЗАЦИЯ СОСТОЯНИЯ ИКОНОК
        if (currentGamePhase === 'revelation') {
            const voteIcon = document.querySelector('.icon-left');
            if (voteIcon) {
                voteIcon.style.pointerEvents = 'none';
    ...
    if (answersContainer) {
        ...
        function checkAllAnswered() {
            ...
            for (let i = 1; i <= total; i++) {
                if (userAnswers[i] && userAnswers[i].answerText) answered++;
            }
            allQuestionsAnswered = (answered === total);
            console.log(`Проверка: отвечено ${answered} из ${total}, allQuestionsAnswered = ${allQuestionsAnswered}`);
            updateButtonState();
        }

        // Обновление ответа для конкретного вопроса
        function updateAnswer(qid, selectedOptionsData) {
            if (selectedOptionsData.length === 0) {
                userAnswers[qid] = { answerText: '', score: 0, comment: '' };
            } else if (qid === 2) {
                // Вопрос 2 – множественный выбор
                const selectedRoles = selectedOptionsData.map(opt => opt.text);
                let answerText = '';
                let comment = '';
                let totalScore = 0;
                if (selectedRoles.length === 6) {
                    answerText = "Есть все 6 ролей";
                    comment = "У нас полный комплект, каждый занимается своим делом";
                    totalScore = selectedOptionsData.reduce((sum, opt) => sum + opt.score, 0);
                } else {
                    answerText = selectedRoles.join(', ');
                    comment = selectedOptionsData.map(opt => opt.comment).join(', ');
                    totalScore = selectedOptionsData.reduce((sum, opt) => sum + opt.score, 0);
                }
                userAnswers[qid] = { answerText, score: totalScore, comment };
            } else {
                // Остальные вопросы – одиночный выбор
                const opt = selectedOptionsData[0];
                userAnswers[qid] = { answerText: opt.text, score: opt.score, comment: opt.comment };
            }
            console.log(`Ответ для вопроса ${qid}:`, userAnswers[qid]);
            checkAllAnswered();
        }
        ...
        function renderQuestions(questionsData) {
            ...
            let answersContent = mainContainer.querySelector('.answers-content');
            if (!answersContent) {
                answersContent = document.createElement('div');
                answersContent.className = 'answers-content';
                mainContainer.appendChild(answersContent);
            }
            ...
            `;
                const optionsDiv = card.querySelector('.answers-options');
                q.options.forEach(opt => {
                    const optDiv = document.createElement('div');
                    optDiv.className = 'answers-option';
                    optDiv.dataset.value = opt.text;
                    optDiv.dataset.score = opt.score;
                    optDiv.dataset.comment = opt.comment;
                    optDiv.innerHTML = `
            ...
            // Находим или создаём кнопку
            let btn = mainContainer.querySelector('.answers-next-btn');
            if (!btn) {
                btn = document.createElement('div');
                btn.className = 'answers-next-btn';
                btn.innerHTML = `
            ...
            // Сброс состояния
            userAnswers = {};
            questionsData.forEach(q => { userAnswers[q.id] = { answerText: '', score: 0, comment: '' }; });
        ...
        function attachSubmitHandler() {
            ...
            submitButton.onclick = async () => {
                console.log('Кнопка нажата, allQuestionsAnswered =', allQuestionsAnswered);
                if (!allQuestionsAnswered) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        // Ждём activate-result-button от сервера
                    } else {
...
if (voteContainer) {
    ...
    showToast(targetUuid ? 'Голос учтен' : 'Вы пропустили голосование', 'success');
